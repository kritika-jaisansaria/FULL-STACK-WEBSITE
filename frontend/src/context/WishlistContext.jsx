...
export const WishlistProvider = ({ children }) => {
  ...
  const fetchWishlist = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8080'}/api/wishlist`, {
...
const toggleWishlist = async (product) => {
  ...
  try {
    if (exists) {
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8080'}/api/wishlist/${product._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8080'}/api/wishlist`,
